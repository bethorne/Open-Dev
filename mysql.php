...
header("content-disposition: attachment;filename=Libro_venta.xls");
...
$sql = "SELECT fecha_doc, tipo_doc, codigo_doc, rut_cli, total_doc,tipo_doc,estado_doc  FROM tbk_documento WHERE codigo_doc <>  '' AND SUBSTR( fecha_doc, 4, 2 ) =  '".$periodo."' AND SUBSTR( fecha_doc, 7, 4 ) =   '".$year."' AND tipo_doc=3 ORDER BY fecha_doc";
...
$d = $ff[0];
			$fechc		= SUBSTR( $row[0], 4, 1 );
				...
				$fichacliente  = explode("|",clienterut($row[3]));
...
</tr>", $row[0],"".$proceso."".$documento,$row[2],"(".$row[3].") - ".$nombre,$neto,$iva,$total);
}
...
header("Cache-Control: must-revalidate, post-check=0, pre-check=0");
header("content-disposition: attachment;filename=Libro_venta_".$fechc."_".$year.".xls");

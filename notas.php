...
<?php
	...
	if (!empty($see))
	{
		...
		SWITCH($see)
		{
			...
			case '1':
					$insert = "SELECT * FROM tbk_nota WHERE id_nota LIKE '%".ltrim($codigofactura)."%' AND tipo_nota = '1' ORDER BY id_nota DESC";
					break;
...
<td valign='top' >
	...
	<form name='boleta' action='notas.php?cb=<?=$cb?>' method ='POST'>
				...
				<?php
					...
					if ($busqueda == 1)
					{
						...
						While ($row = mysql_fetch_row($res))
						{
							...
							echo "<tr><td id='data' width='20'><a id='etiquetazul' href='notas.php?cb=".$guiaID."'><font size='4'>".$guiaID."</font></a></td><td id='data' align='center'><b>".$codigoID."</b></td><td id='data'>".$fechaguia."</td><td id='data' align='right'>$ ".$total."</td></tr>";
							$i++;
						}
					}
					else
	...
	<?php if (!empty($cb)) :?>
			...
			<td   id='data' align='left' colspan='6'>
				 <label id='comentario'><font size='3'> Factura N°:  <?=codigoid($sfactura)?></font> - <a id='etiquetazul' href='facturas.php?cb=<?=$sfactura?>' target='_blank'> <img src='images/binoculares.gif' border='0'></a></label>
			</td>
			</tr>

...
<?php
	...
	if (!empty($see))
	{
		...
		SWITCH($see)
		{
			...
			case '1':
					$insert = "SELECT * FROM tbk_arriendo WHERE id_doc LIKE '%".ltrim($codigofactura)."%' AND tipo_doc = 0  ORDER BY id_doc DESC";
					break;
					
		
			case '2':
					$insert = "SELECT * FROM tbk_arriendo WHERE rut_cli LIKE '".trim($nrut)."' AND tipo_doc = 0 ORDER BY id_doc DESC";
					break;
					
		

			case '3':
					$insert = "SELECT * FROM tbk_arriendo WHERE SUBSTR(fecha_doc,4,2) LIKE '%".ltrim($marca)."%' AND tipo_doc = 0 ORDER BY id_doc DESC";
					break;
			
			case 'save':
						
			
				$insert ="UPDATE  tbk_docarr SET  cantidad_fp =  '".$can."',valor_fp =  '".$uni."' 	WHERE  id_doc ='".$cb."' ";
				break;
			case '9':
...
<td valign='top' >
	...
	<form name='boleta' action='valesarriendo.php?cb=<?=$cb?>' method ='POST'>
				...
				<?php
					...
					if ($busqueda == 1)
					{
						...
						While ($row = mysql_fetch_row($res))
						{
							...
							echo "<tr><td id='data' width='20'><a id='etiquetazul' href='valesarriendo.php?cb=".$facturaID."'><font size='4'>".$facturaID."</font></a></td><td id='data'>".$nombrecliente."</td><td id='data'>".$fechafact."</td><td id='data'>$ ".$total."</td><td id='data'>".$proceso."</td></tr>";
							$i++;
						}
					}
					else
	...
	<?php if (!empty($cb)) :?>
			...
			<tr><td id='etiqueta'> Producto</td><td id='etiqueta'> Cant.</td><td   width='30' >   </td><td id='etiqueta' width='30' >Precio Venta</td> </tr>
			...
			<?php
				...
				?>	
					...
					<td id='data' align='right' width='10'>
						<input id='comentario' value="<?=$fcantidad?>" size="6" name='cant' onkeypress="return event.keyCode!=13" >
					</td>
                    <td id='data' align='right' width='10'>
						<input type='button' value='=' onClick='calculararriendo()'>
					</td>
					<td id='data' align='right' >
						<input id='comentario' name ='valoruni' value="<?=$fvalorunitario?>">
					</td>
					<td   >
						
                        <input  type='hidden' name='totalarr'value='<?= $ $stotal?>' size='12' />
...
-->
					...
					<td id='etiqueta' align='right' width='120' >
						$
						<input  type='text' name='totalf'value='<?=$stotal?>' size='12' />
			...
			<a href="vervalearriendo.php?cb=<?=$cb?>" onClick="window.open(this.href, this.target, 'width=500,height=400'); return false;">Imprimir</a>
			</fieldset>
	...
	<td valign='top'>
			...
			<fieldset>
				...
				<td id='data'>
					<input type='text' name='descuentovale' value='0' size='20' style='text-align:right' onkeypress="return event.keyCode!=13"><input type='button' value='=' onClick='calculadescuento()'>
				</td>
				</tr>
			...
			<p/>
			<label id='subtitulo'> Administraci&oacute;n de Cuotas </label>
			<br/>
			...
			<p/>
				...
				<td align='right'>
					...
					<input type='submit' value='Aceptar' onClick='document.boleta.action = "procesavalearriendo.php"; submit();'>
				</td>
				</tr>
				</table>

...
<?php
	...
	if (!empty($see))
	{
		...
		SWITCH($see)
		{
			...
			case '1':
					$insert = "SELECT * FROM tbk_documento WHERE id_fact doc '%".ltrim($codigofactura)."%' AND tipo_doc = 3 ORDER BY id_fact DESC";
					break;
...
<td valign='top' >
	...
	<?php if (!empty($cb)) :?>
			...
			<tr><td id='etiqueta'> Producto</td><td id='etiqueta'> Cant.</td><td id='etiqueta' width='30' >  Valor</td><td id='etiqueta' width='30' >  Total</td></tr>
			...
			<?php
				...
				?>
					...
					<td />
					<td >
						<label id='comentario'><b>NETO</b> </label>
					</td>
					<td id='etiqueta' align='right' colspan='2'>
						$ <?=$subtotal?>
					</td>
					</tr>	
					...
					<td />
					<td  >
						<label id='comentario'><b>IVA</b> </label>
					</td>

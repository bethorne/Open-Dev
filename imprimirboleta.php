...
<?php
	...
	if (!empty($see))
	{
		...
		SWITCH($see)
		{
			...
			case '1':
					$insert = "SELECT * FROM tbk_documento WHERE codigo_doc LIKE '".trim($codigofactura)."%' AND tipo_doc = 1 AND codigo_doc  <> '' ORDER BY id_doc DESC";
					break;
					
		
			case '2':
					$insert = "SELECT * FROM tbk_documento WHERE rut_cli LIKE '".trim($nrut)."%' AND tipo_doc IN (1) AND codigo_doc  <> '' ORDER BY id_doc DESC";
					break;
					
		

			case '3':
					$insert = "SELECT * FROM tbk_doc WHERE SUBSTR(fecha_doc,4,2) LIKE '%".ltrim($marca)."%' AND tipo_doc IN (1,2,3) AND codigo_doc  <> '' ORDER BY id_doc DESC";
					break;
...
<td valign='top' >
	...
	<label id='subtitulo'> IMPRESION DE DOCUMENTO DE VENTA :: BOLETA</label>
	<br/>
	...
	<form name='boleta' action='imprimirboleta.php?cb=<?=$cb?>' method ='POST'>

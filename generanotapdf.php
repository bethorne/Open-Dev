...
global $rutcli,$stipodoc,$fechadoc,$codigodoc, $codigofac ,$vendedor,$formadepago;
	// buscar informacion de Factura

	$bf= "SELECT * FROM  tbk_nota WHERE id_nota='".$cb."'";
	...
	$bfac= "SELECT rut_cli, codigo_doc, vendedor_doc FROM  tbk_documento WHERE id_doc='".$factura."'";
	$rfac = mysql_query($bfac, $conn);
	
	$fichafac = mysql_fetch_row($rfac);

	
	
	$GLOBALS['rutcli']		= $fichafac[0];
	$GLOBALS['nombrecli']   	= clienterut($GLOBALS['rutcli']);
	$stipodoc 			= $ficha[3];
	$GLOBALS['fechadoc'] 	= $ficha[2];
	$GLOBALS['codigofact'] 	= $fichafac[1];
	$GLOBALS['codigodoc']       = $cb;
	$GLOBALS['vendedor']  	= $fichafac[2];
...
function Header()
{
	...
	$this->Cell(188,18,'',0,1,'R');
	$this->Cell(172,17,'',0,1,'R');
	$this->SetFont('Arial','B',10);
    	$this->Cell(172,5,$GLOBALS['codigodoc'],0,0,'R');
	$this->Cell(20,5,'F: '.$GLOBALS['codigofact'] ,0,1,'L');
...
$find = "SELECT * FROM tbk_notapro WHERE  id_nota ='".$cb."'";

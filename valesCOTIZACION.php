...
<?php
	...
	if (!empty($see))
	{
		...
		SWITCH($see)
		{
			...
			case '1':
					$insert = "SELECT * FROM tbk_cotizacion WHERE id_doc LIKE '%".ltrim($codigofactura)."%' AND tipo_doc = 0  ORDER BY id_doc DESC";
					break;
					
		
			case '2':
					$insert = "SELECT * FROM tbk_cotizacion WHERE rut_cli LIKE '".trim($nrut)."' AND tipo_doc = 0 ORDER BY id_doc DESC";
					break;
					
		

			case '3':
					$insert = "SELECT * FROM tbk_cotizacion WHERE SUBSTR(fecha_doc,4,2) LIKE '%".ltrim($marca)."%' AND tipo_doc = 0 ORDER BY id_doc DESC";
					break;
...
<td valign='top' >
	...
	<form name='boleta' action='valesCOTIZACION.php?cb=<?=$cb?>' method ='POST'>
				...
				<?php
					...
					if ($busqueda == 1)
					{
						...
						While ($row = mysql_fetch_row($res))
						{
							...
							echo "<tr><td id='data' width='20'><a id='etiquetazul' href='valesCOTIZACION.php?cb=".$facturaID."'><font size='4'>".$facturaID."</font></a></td><td id='data'>".$nombrecliente."</td><td id='data'>".$fechafact."</td><td id='data'>$ ".$total."</td><td id='data'>".$proceso."</td></tr>";
							$i++;
						}
					}
					else

...
<?php
	...
	if (!empty($see))
	{
		...
		SWITCH($see)
		{
			...
			case '1':
					$insert = "SELECT * FROM tbk_documento WHERE id_doc LIKE '".trim($codigofactura)."%' AND tipo_doc = 2 AND codigo_doc  <> '' ORDER BY id_doc DESC";
					break;
					
		
			case '2':
					$insert = "SELECT * FROM tbk_documento WHERE rut_cli LIKE '".trim($nrut)."%' AND tipo_doc IN (2) AND codigo_doc  <> '' ORDER BY id_doc DESC";
					break;
...
<td valign='top' >
	...
	<label id='subtitulo'> IMPRESION DE DOCUMENTO DE VENTA :: GUIA</label>
	<br/>
	...
	<form name='boleta' action='imprimirguia.php?cb=<?=$cb?>' method ='POST'>
	...
	<td width='400' valign='top'>
			...
			<p/>
				...
				<tr>
				<td>
					<label id='comentario'>C&oacute;digo Gu&iacute;a</label>
				</td>

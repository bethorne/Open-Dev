...
<?php
	...
	if (!empty($see))
	{
		...
		SWITCH($see)
		{
			...
			case '1':
					$insert = "SELECT * FROM tbk_documento WHERE codigo_doc LIKE '".trim($codigofactura)."%' AND tipo_doc IN (1,2,3)  AND codigo_doc  <> '' ORDER BY id_doc DESC";
					break;
					
		
			case '2':
					$insert = "SELECT * FROM tbk_documento WHERE rut_cli LIKE '".trim($nrut)."%' AND tipo_doc IN (1,2,3) AND codigo_doc  <> '' ORDER BY id_doc DESC";
					break;
					
		

			case '3':
					$insert = "SELECT * FROM tbk_doc WHERE SUBSTR(fecha_doc,4,2) LIKE '".trim($marca)."%' AND tipo_doc IN (1,2,3) AND codigo_doc  <> '' ORDER BY id_doc DESC";
					break;
...
<td valign='top' >
	...
	<label id='subtitulo'> BUSCAR DOCUMENTO DE VENTA</label>
	<br/>
	...
	<td width='400' valign='top'>
			...
			<label id='subtitulo'> Buscar y seleccionar documentos de venta </label>
			<p/>
				...
				<?php
					...
					if ($busqueda == 1)
					{
						...
						While ($row = mysql_fetch_row($res))
						{
							...
							$nombrecliente = $nc[0];
							echo "<tr><td id='data' width='20'><a id='etiquetazul' href='facturas.php?cb=".$facturaID."' target='_blank'><font size='4'>".$codigodoc."</font></a></td><td id='data'>".$nombrecliente."</td><td id='data'>".$fechafact."</td><td id='data'>$ ".$total."</td><td id='data'>".$proceso."</td></tr>";
							$i++;
						}
					}
					else
				...
				?>	
					...
					<td id='data' align='right' >
						...
						<?php 
							...
							if ($stipodoc == 4) echo $fvalorventa;
					...
					<td id='data' align='right' >
						...
						<?php 
							...
							if ($stipodoc == 4) echo $fvalorventa*$fcantidad;
					...
					if ($stipodoc == 3)
					{
						...
						if ($notas >= 1)
						{
							...
							WHILE ($fichanota  = mysql_fetch_row($resnotas))
							{
								...
								{
									case '0' : 
												echo "<tr><td id='etiquetacredito' colspan='5'> Nota Cr&eacute;dito N° ".$idnota."</td>";
												break;
									case '1' : 
												echo "<tr><td id='etiquetadebito' colspan='5'> Nota D&eacute;bito N° ".$idnota."</td>";
												break;
					...
					<?php
							...
							{	
								...
								WHILE ($ficha4 = mysql_fetch_row($resst2))
								{
									...
									echo "<tr>";
									echo "<td id='etiqueta' width='150'>GU&Iacute;A N° ".$campo2." </td><td id='data'>".$campo3." </td><td id='data' align='center'> <a id='etiquetazul' href='guias.php?cb=".$campo2."'><img src='images/lupa.png' border='0'></a></td> ";

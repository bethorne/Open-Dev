	...
	if (!empty($see))
	{
		...
		SWITCH($see)
		{
			...
			case '1':
					if ($codigofactura == '')
					{
						$insert = "SELECT * FROM tbk_documento WHERE tipo_doc = 1  AND codigo_doc <> '' AND estado_doc = 1 ORDER BY id_doc DESC";
	
					}
					else
					{
						$insert = "SELECT * FROM tbk_documento WHERE codigo_doc LIKE '".trim($codigofactura)."%' AND tipo_doc = 1   AND estado_doc = 1 ORDER BY codigo_doc DESC";	
					}
					break;
					
		
			case '2':
					$insert = "SELECT * FROM tbk_documento WHERE rut_cli LIKE '".trim($nrut)."%' AND estado_doc = 1  AND tipo_doc = 1  ORDER BY id_doc DESC";
					break;
...
<td valign='top' >
	...
	<label id='subtitulo'> PAGO DE BOLETA</label>
	<br/>
	<label id='comentariogris'> Complete uno de los campos para realizar la b&uacute;squeda. Si desea listar todos las boletas, haga click en 'buscar c&oacute;digo' con el campo vac&iacute;o.</label>
	<hr/>
	<p/>


	<form name='boleta' action='pagos3.php?cb=<?=$cb?>' method ='POST'>
	...
	<td width='400' valign='top'>
			...
			<label id='subtitulo'> Resultado B&uacute;queda </label>
				...
				<tr>
				<td>
					<label id='comentario'>C&oacute;digo Boleta</label>
				</td>
				...
				<?php
					...
					if ($busqueda == 1)
					{
						...
						While ($row = mysql_fetch_row($res))
						{
							...
							echo "<tr><td id='data' width='20'><a id='etiquetazul' href='pagos3.php?cb=".$facturaID."'><font size='4'>".$codigodoc."</font></a></td><td id='data'>".$clienteID."</td><td id='etiqueta'>".$folio."</td><td id='data'>".$fechafact."</td><td id='data'>$ ".$total."</td></tr>";
							$i++;
						}
					}
					else
	...
	<?php if (!empty($cb)) :?>
		...
		?>	
					...
					<p/>
					<label id='subtitulo'> Datos de Boleta </label>
					<p/>
					...
					<fieldset>
						...
						<td id='etiqueta'  align='right' >
							$ <?=$vectorcuotas[($d)]?>
							<input type='hidden' name='montopagoboleta' value='<?=$vectorcuotas[($d)]?>'>
						</td>
						</tr>
						</table>
					...
					<table border='0' cellspacing='5' cellpadding='5' width='810'>
						<tr>
						<td />
						<td align='right'>
								<table border='0'>

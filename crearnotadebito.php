...
<?php
	...
	WHILE ($rowparam = mysql_fetch_row($resultadocodigo))
	{
		...
		if ($rowparam[2] =='notadebito')  $nota 	= $rowparam[4] ;
	...
	*/
	...
	if (!empty($see))
	{
		...
		SWITCH($see)
		{
			...
			case '1':
					$insert = "SELECT * FROM tbk_documento WHERE id_doc LIKE '%".ltrim($codigofactura)."%' AND tipo_doc = 3 ORDER BY id_doc DESC";
			...
			case '9':
					...
					if($resulta = mysql_query($insert, $conn))
					{
							...
							$update = "UPDATE tbk_parametro SET valor_param =".($codigonota + 1)." WHERE id_param =5";
							//echo "<p>".$update."<p>";
							...
							WHILE ($prod = $cbarradespacho[$p])
							{
								...
								if ($resulta = mysql_query($insert, $conn))
								{
									...
									if ($flagcuotas == 0)
									{
										...
										// procedimiento valor nota debito
										// 1. Consultar cantidad de cuotas de factura
										// 2. Crear una nueva cuota que incluya el valor de la nota de debito
										// 3. Agregar 1 mes a la mayor fecha
										// 4. agregar a cuotas de factura con la fecha de vencimiento el mes siguiente
										
										$buscacuotas  = "SELECT  id_fact, rut_cli, vencimiento_pc FROM tbk_pago_cuota  WHERE id_fact ='".$cb."'";
										if ($rescuotas = mysql_query($buscacuotas, $conn))
										{
											...
											if ($cantidadcuotas > 0)
											{
												...
												//echo "<p> ULTIMA FECHA: ".$fechafinal.",".$ffechafinal." </p>";
												
												$fecha = explode("-",$ffechafinal);
													
												$y = $fecha[2];
												$m = $fecha[1];
												$d = $fecha[0];
												
												$sumaunmes = mktime ( 0, 0, 0, date("$m") + 1, date("$d"), date("$y"));
												
												
												//4 
												$insertc="INSERT INTO tbk_pago_cuota VALUES(";
												$insertc.="'".$cb."',";
												$insertc.="'".$rutcli."',";
												$insertc.="'',";
												$insertc.="'".$cuotadebito."',";
												$insertc.="'".date('d-m-Y',$sumaunmes)."',";
												$insertc.="'',";
												$insertc.="'0',";
												$insertc.="'0',";
												$insertc.="'".$cuotadebito."'";
										...
										if ($rescuotas = mysql_query($buscacuotas, $conn))
										{
											...
											echo "<p>CUOTA DEBITO :: ".$cuotadebito."</p>";
									...
									*/	
									...
									//echo $insertkdx;
									if ($reskdx  = mysql_query($insertkdx, $conn)) echo "";
...
<td valign='top' >
	...
	<form name='boleta' action='crearnotadebito.php?cb=<?=$cb?>' method ='POST'>
					...
					<?php
						...
						if ($busqueda == 1)
						{
							...
							SWITCH($tipodoc)
							{
								...
								echo "<tr><td id='data' width='20'><a id='etiquetazul' href='crearnotadebito.php?cb=".$facturaID."'><font size='4'>".$codigodoc."</font></a></td><td id='data'>".$clienteID."</td><td id='data'>".$fechafact."</td><td id='data' align='right'>$ ".$total."</td><td id='data'>".$proceso."</td></tr>";
								$i++;
							}
						}
						else
	...
	<?php if (!empty($cb)) :?>
		...
		<?php if (($guiaregistrada == 0)) :?>		
				...
				<?php if (!empty($cb)) :?>
						...
						<label id='subtitulo'> Nota de D&eacute;bito N° <?= $nota?></label>
						<p/>
						...
						<td  id='data' align='right' colspan='8'>
							 <label id='comentario'> Nota de D&eacute;bito N°<br/> <font size='4'>  <?=$nota?></font></label>
						</td>
						</tr>
						...
						<tr><td id='etiqueta'> Producto</td><td id='etiqueta' >Cantidad</td><td id='etiqueta' > Retiro</td><td id='etiqueta' >PRECIO NETO</td> <td id='etiqueta' width='5'/><td id='etiqueta' >$ TOTAL</td></tr>
						...
						<td align='right'>
								...
								<input type='hidden' name='codigonota' value='<?=$nota?>'>

...
include("../baro/conector/conector.php");
			...
			?>
          <table  border='0'   width="350"   cellspacing='1' cellpadding='1'>  
           <tr>
		 
		<td width="643" id='etiqueta'> PRODUCTO</TD>
		<td width="150" id='etiqueta'> CANTIDAD VENDIDA</TD>
...
$strXML = "<chart caption = 'Producto mas Vendidos ' bgColor='#CDDEE5' baseFontSize='12' showValues='1' xAxisName='Meses' formatNumberScale='0'>";
...
$searchgv="SELECT  `fecha_kdx` ,  `id_pro` , COUNT(  `id_pro` ) 
FROM tbk_kardex
WHERE  `operacion_kdx` =1
   GROUP BY  `id_pro` 
ORDER BY COUNT(  `id_pro` ) DESC  ";
		...
		WHILE($linea = mysql_fetch_row($resp))
			{
				...
				$DSA 			=$linea[2];
			$fk  = substr($fecha,0,4)."-".substr($fecha,4,2)."-".substr($fecha,6,2)." ".substr($fecha,8,2).":".substr($fecha,10,2);
$strXML .= "<set  label ='".$fk."'  value ='".$intTotalAnio1."' color = 'EA1000' />";

$buscapro = "SELECT  nombre_pro  FROM tbk_producto WHERE id_pro ='".trim($intTotalAnio1)."'";
...
<table  border='0' width="350"  cellspacing='1' cellpadding='1'>
  ...
  <tr>
    <td id='etiqueta'><?php echo"$nombrepro ";?></td>
    <td id='data'><?php echo"$DSA ";?></td>
  </tr>
</table>

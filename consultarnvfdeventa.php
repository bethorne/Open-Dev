...
<?php
	...
	if (($periodo != '') AND ($yearguia != ''))
	{
		...
		if (empty($cliente))
		{
			...
			$searchgv = "SELECT * FROM tbk_documento WHERE tipo_doc = 4  AND estado_doc = 0 AND SUBSTR(fecha_doc, 7,4) = '".$yearguia."'  AND SUBSTR(fecha_doc, 4,2) ='".$periodo."' 
ORDER BY  `tbk_documento`.`rut_cli`  ASC";
		}
		else
			{
...
<form action='consultarnvfdeventa.php' name='gv' method='POST'>
	...
	<?php 
		...
		if ($busca == 1)
		{
			...
			WHILE ($rowgv = mysql_fetch_row($resgv))
			{
				...
				echo "	<td id='etiqueta' align='center'></td>";

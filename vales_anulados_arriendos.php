...
<?php
	...
	$searchcodigo = "SELECT * FROM tbk_arriendo WHERE  tipo_param = 3";
	...
	if (!empty($see))
	{
		...
		SWITCH($see)
		{
			...
			case '1':
					$insert = "SELECT * FROM tbk_arriendo WHERE id_doc LIKE '%".ltrim($codigofactura)."%' AND tipo_doc = 0 AND estado_doc = 9 ORDER BY id_doc DESC";
					break;
					
		
			case '2':
					$insert = "SELECT * FROM tbk_arriendo WHERE rut_cli LIKE '%".trim($nrut)."%' AND tipo_doc = 0  AND estado_doc = 9 ORDER BY id_doc DESC";
					break;
					
		

			case '3':
					$insert = "SELECT * FROM tbk_arriendo WHERE SUBSTR(fecha_doc,4,2) LIKE '%".ltrim($marca)."%' AND tipo_doc = 9 AND estado_doc = 0 ORDER BY id_doc DESC";
					break;
...
<td valign='top' >
	<br/>
	<p/>
	<label id='subtitulo'>ANULAR VALE DE ARRIENDO</label>
	<br/>
	...
	<form name='boleta' action='vales_anulados_arriendos.php?cb=<?=$cb?>' method ='POST'>
				...
				<?php
					...
					if ($busqueda == 1)
					{
						...
						While ($row = mysql_fetch_row($res))
						{
							...
							echo "<tr><td id='atrasocuota' width='20'><a id='etiquetazul' href='vales_anulados_arriendos.php?cb=".$facturaID."'><s><font size='4'>".$facturaID."</font></s></a></td><td id='data'>".$clienteID."</td><td id='data'>".$fechafact."</td><td id='data'>$ ".$total."</td><td id='data'>".$proceso."</td></tr>";
							$i++;
						}
					}
					else
